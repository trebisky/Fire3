...
struct	nx_clkpwr_registerset {
	...
	u32 dvoreg[9];			/* 0x020 ~ 0x040 : Divider
					   Setting Register */
	...
	u32 gpiowakeupriseenb;		/* 0x200 : GPIO Rising Edge
					   Detect Enable Register */
	u32 gpiowakeupfallenb;		/* 0x204 : GPIO Falling Edge
					   Detect Enable Register */
	...
	u32 scratch[3];			/* 0x230 ~ 0x238	:
						   Scratch Register */
	u32 sysrstconfig;		/* 0x23C : System Reset
						   Configuration Register. */
	u8  __reserved6[0x2A0-0x240];	/* padding (0x240 ~ 0x29F) */
	u32 cpupowerdownreq;		/* 0x2A0 : CPU Power Down
						   Request Register */
	u32 cpupoweronreq;		/* 0x2A4 : CPU Power On Request
						   Register */
	u32 cpuresetmode;		/* 0x2A8 : CPU Reset Mode Register */
	u32 cpuwarmresetreq;		/* 0x2AC : CPU Warm Reset
						   Request Register */
...
long clk_round_rate(struct clk *clk, unsigned long rate)
{
	...
	step = peri->clk_step;
	mask = peri->in_mask;
	debug("clk: %s.%d reqeust = %ld [input=0x%x]\n", peri->dev_name,
